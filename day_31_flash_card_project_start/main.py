...
try:
    data = pandas.read_csv(filepath_or_buffer= parent_folder_path.joinpath('data/words_to_learn.csv'))
except FileNotFoundError:
    original_data = pandas.read_csv(parent_folder_path.joinpath('data/indonesian_words.csv'))
...
card_front_img = PhotoImage(file = parent_folder_path.joinpath('images/card_front.png'))
card_back_img = PhotoImage(file = parent_folder_path.joinpath('images/card_back.png'))
...
def is_known():
    ...
    data.to_csv(parent_folder_path.joinpath('data/words_to_learn.csv'), index=False)
    next_card()
...
cross_image = PhotoImage(file = parent_folder_path.joinpath('images/wrong.png'))
unknown_btn = Button(image=cross_image, highlightthickness=0, relief='flat', bg=BACKGROUND_COLOR, command=next_card)
unknown_btn.grid(row=1, column=0)

check_image = PhotoImage(file = parent_folder_path.joinpath('images/right.png'))
